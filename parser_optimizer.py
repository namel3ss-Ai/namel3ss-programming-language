...
page "Chat" at "/chat" {
    ...
    for mult in multipliers:
        ...
        for _ in range(10):
            ...
            try:
                parser = Parser(content, path=f"test_{mult}.n3")
                parser.parse()
        ...
        for _ in range(10):
            start = time.perf_counter()
            try:
                parser = OptimizedParser(content, path=f"test_{mult}.n3")
                parser.parse()
                end = time.perf_counter() 
    ...
    parser1 = OptimizedParser(test_content, path="cache_test.n3")
    start = time.perf_counter()
    module1 = parser1.parse()
    first_parse_time = time.perf_counter() - start
    
    # Second parse (cache hit)
    parser2 = OptimizedParser(test_content, path="cache_test.n3")
    start = time.perf_counter()
