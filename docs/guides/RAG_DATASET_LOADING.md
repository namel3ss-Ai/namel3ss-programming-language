...
**Build the index:**
```bash
namel3ss build-index my_app.ai docs_index --verbose
```
...
```bash
namel3ss build-index app.ai my_index --max-documents 1000
# or short form:
namel3ss build-index app.ai my_index -n 1000
```

### Filter by Metadata
```bash
# Single filter
namel3ss build-index app.ai my_index --filter category=support

# Multiple filters
namel3ss build-index app.ai my_index --filter category=support --filter lang=en --filter priority=high
```
...
namel3ss build-index app.ai my_index --resume --verbose
```

### Force Rebuild
```bash
# Delete checkpoint and rebuild from scratch
namel3ss build-index app.ai my_index --force-rebuild --verbose
```
...
```bash
namel3ss build-index app.ai my_index --resume
```

To force rebuild (ignoring previous state):
```bash
namel3ss build-index app.ai my_index --force-rebuild
```
...
```bash
namel3ss build-index app.ai my_index --dataset my_dataset
```
...
# NEW (production):
# Just define your dataset in .ai file and run CLI
