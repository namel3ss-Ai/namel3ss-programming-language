...
The Namel3ss CLI compiles `.ai` source into a runnable [Runtime](docs/reference/GLOSSARY.md#runtime), serves it for development, and executes evaluation hooks. This guide targets experienced engineers and uses terminology defined in the [Glossary](docs/reference/GLOSSARY.md).
...
## Command Summary
- `namel3ss build`: Compile `.ai` into frontend and backend runtime assets.
...
- `--verbose` (global): Emit full tracebacks for debugging (or set `NAMEL3SS_VERBOSE=1`; `NAMEL3SS_RERAISE=1` re-raises).
...
namel3ss build app.ai --out dist

# Backend + frontend
namel3ss build app.ai --build-backend --backend-out api

# Backend only
namel3ss build app.ai --backend-only --backend-out runtime_api
```

> **Best Practice**  
> Use `--env KEY=VALUE` to provide secrets at build time. Do not hard-code provider credentials in `.ai` files.

## namel3ss run
Compile and start a development server for a single `.ai` file.
...
```bash
namel3ss run app.ai
namel3ss run app.ai --port 3000 --host 0.0.0.0
namel3ss run app.ai --backend-out dev_backend --no-reload
```
...
## namel3ss eval
Execute experiment variants declared in `.ai`.
...
**Options**
- `-f, --file <path>`: Path to `.ai` (default: first `.ai` in the working directory).
...
```bash
namel3ss run myapp.ai
```

3. Make changes to your `.ai` file - the server will automatically reload
...
```bash
namel3ss build myapp.ai --build-backend --out frontend --backend-out backend
```
...
```bash
namel3ss build myapp.ai --print-ast > ast.json
```
...
### Backend-Only Builds
```bash
namel3ss build app.ai --backend-only --backend-out api
```
