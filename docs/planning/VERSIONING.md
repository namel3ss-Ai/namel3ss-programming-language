...
1. **Language Level**: The `.ai` syntax and semantics
...
A MAJOR version increment (e.g., `1.x.x` → `2.0.0`) indicates **breaking changes** that may require you to update your `.ai` files or generated code.

#### Language-Level Breaking Changes

- Removal of syntax constructs (e.g., deprecating a keyword)
- Changes to parsing rules that reject previously valid `.ai` files
...
- **✅ GUARANTEED**: Any `.ai` file that compiled successfully in `1.0.0` will compile successfully in `1.9.0`
...
- A `.ai` file using features from `1.5.0` may not compile with `1.3.0`
...
namel3ss build app.ai         # Generates 1.5.0 backend
```
...
namel3ss build app.ai  # Regenerate to get bug fixes
```
