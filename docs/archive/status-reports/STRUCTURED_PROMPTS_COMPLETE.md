...
The structured prompts feature is now fully integrated into the grammar parser (`namel3ss/lang/grammar.py`), enabling end-to-end compilation of `.ai` files via CLI.
...
- ✅ Verified E2E: `.ai file → CLI build → AST with structured fields`

**Verification:**
```bash
cd namel3ss-programming-language
python -m namel3ss.cli build test_structured_final/app.ai --print-ast
```
...
- **Main Documentation**
  ...
  - `examples/structured_prompt_demo.ai` - Example application
