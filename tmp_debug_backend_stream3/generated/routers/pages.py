...
@router.get('/api/pages/root' + '/tables/0', response_model=TableResponse, tags=['pages'])
async def dashboard_table_0(session: AsyncSession = Depends(get_session)) -> TableResponse:
    ...
    if runtime.REALTIME_ENABLED and is_reactive:
        await runtime.broadcast_component_update('dashboard', 'table', 0, response, meta={'source': 'orders'})
    return response
