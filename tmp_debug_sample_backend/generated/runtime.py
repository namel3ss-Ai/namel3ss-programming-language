...
DATASETS: Dict[str, Dict[str, Any]] = {'monthly_sales': {'name': 'monthly_sales',
                   'source_type': 'table',
                   'source': 'sales',
                   'operations': [{'type': 'group_by', 'columns': ['month']},
                                  {'type': 'aggregate',
                                   'function': 'sum',
                                   'expression': 'revenue as total_revenue'},
                                  {'type': 'computed_column',
                                   'name': 'revenue_plus_tax',
                                   'expression': 'revenue + tax',
                                   'expression_expr': {'type': 'binary',
                                                       'op': '+',
                                                       'left': {'type': 'name', 'name': 'revenue'},
                                                       'right': {'type': 'name', 'name': 'tax'}}},
                                  {'type': 'filter',
                                   'condition': 'revenue_plus_tax > 100',
                                   'condition_expr': {'type': 'binary',
                                                      'op': '>',
                                                      'left': {'type': 'name',
                                                               'name': 'revenue_plus_tax'},
  ...
  'components': [{'type': 'text', 'payload': {'text': 'Hello', 'styles': {}}, 'index': 0},
                 ...
                 {'type': 'action',
                  'payload': {'name': 'Refresh',
                              ...
                              'operations': [{'type': 'toast', 'message': 'Refreshing'}]},
                  'index': 3}]}]
...
async def page_dashboard_0(session: Optional[AsyncSession] = None) -> Dict[str, Any]:
 ...
 {'type': 'action',
  ...
  'operations': [{'type': 'toast', 'message': 'Refreshing'}]}]
    ...
    components = await prepare_page_components({'api_path': base_api_path, 'slug': 'dashboard'}, components, context, session)
    page_errors: List[Dict[str, Any]] = []
    for entry in _collect_runtime_errors(context):
        ...
        scope_value = entry.get('scope')
        normalized_scope = str(scope_value).strip().lower() if scope_value is not None else ''
        if normalized_scope in {'', 'dashboard', 'page:dashboard', 'page.dashboard', 'page'}:
            entry['scope'] = 'page:dashboard'
        page_errors.append(entry)
    return {
        'name': 'Dashboard',
        'route': '/',
