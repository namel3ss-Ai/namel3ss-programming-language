...
APP: Dict[str, Any] = {'name': 'Demo API', 'database': 'DEMO_DB', 'theme': {}, 'variables': []}
DATASETS: Dict[str, Dict[str, Any]] = {'active_users': {'name': 'active_users',
                  'source_type': 'table',
                  'source': 'users',
                  'operations': [{'type': 'filter', 'condition': "status == 'active'"}],
                  'transforms': [],
                  'schema': [],
                  'features': [],
                  'targets': [],
                  ...
                  'sample_rows': [{'id': 1, 'value': 10},
                                  {'id': 2, 'value': 20},
                                  {'id': 3, 'value': 30}]}}
CONNECTORS: Dict[str, Dict[str, Any]] = {'active_users': {'type': 'table', 'name': None, 'options': {}}}
...
PAGES: List[Dict[str, Any]] = [{'name': 'Home',
  ...
  'components': [{'type': 'text',
                  'payload': {'text': 'Welcome to Demo API', 'styles': {}},
                  'index': 0},
                 {'type': 'table',
                  'payload': {'title': 'Active Users',
                              'source_type': 'dataset',
                              'source': 'active_users',
                              'columns': [],
                              'filter': None,
 ...
 {'name': 'Admin',
  ...
  'layout': {},
  'components': [{'type': 'if',
                  'payload': {'condition': {'type': 'binary',
                                            'op': '==',
                                            'left': {'type': 'attribute', 'path': ['user', 'role']},
                                            'right': {'type': 'literal', 'value': 'admin'}},
                              'body': [{'type': 'text', 'text': 'Admin Dashboard', 'styles': {}},
                                       {'type': 'table',
                                        'title': 'All Users',
                                        'source_type': 'table',
                                        'source': 'users',
...
async def page_home_0(session: Optional[AsyncSession] = None) -> Dict[str, Any]:
    ...
    scope = ScopeFrame()
    scope.set('context', context)
    instructions = [{'type': 'text', '__component_index': 0, 'text': 'Welcome to Demo API', 'styles': {}},
 {'type': 'table',
  '__component_index': 1,
  'title': 'Active Users',
  'source_type': 'dataset',
...
async def page_admin_1(session: Optional[AsyncSession] = None) -> Dict[str, Any]:
    context = build_context('admin')
    scope = ScopeFrame()
    scope.set('context', context)
    instructions = [{'type': 'if',
  '__component_index': 0,
  ...
  'body': [{'type': 'text', 'text': 'Admin Dashboard', 'styles': {}},
           {'type': 'table',
            ...
            'dynamic_columns': None}],
  'elifs': [],
  'else_body': [{'type': 'text', 'text': 'Access Denied', 'styles': {}}]}]
    components = await render_statements(instructions, context, scope, session)
    base_api_path = '/api/pages/admin'
    components = await prepare_page_components({'api_path': base_api_path, 'slug': 'admin'}, components, context, session)
    page_errors: List[Dict[str, Any]] = []
    for entry in _collect_runtime_errors(context):
        if not isinstance(entry, dict):
            continue
        scope_value = entry.get('scope')
        normalized_scope = str(scope_value).strip().lower() if scope_value is not None else ''
        if normalized_scope in {'', 'admin', 'page:admin', 'page.admin', 'page'}:
            entry['scope'] = 'page:admin'
        page_errors.append(entry)
    return {
        'name': 'Admin',
        'route': '/admin',
