...
APP: Dict[str, Any] = {'name': 'Simple Test', 'database': None, 'theme': {}, 'variables': []}
...
AI_PROMPTS: Dict[str, Dict[str, Any]] = {}
...
async def page_about_1(session: Optional[AsyncSession] = None) -> Dict[str, Any]:
    context = build_context('about')
    scope = ScopeFrame()
    scope.set('context', context)
    instructions = []
    components = await render_statements(instructions, context, scope, session)
    base_api_path = '/api/pages/about'
    components = await prepare_page_components({'api_path': base_api_path, 'slug': 'about'}, components, context, session)
    page_errors: List[Dict[str, Any]] = []
    for entry in _collect_runtime_errors(context):
        if not isinstance(entry, dict):
            continue
        scope_value = entry.get('scope')
        normalized_scope = str(scope_value).strip().lower() if scope_value is not None else ''
        if normalized_scope in {'', 'about', 'page:about', 'page.about', 'page'}:
            entry['scope'] = 'page:about'
        page_errors.append(entry)
    return {
        'name': 'About',
        'route': 'about',
