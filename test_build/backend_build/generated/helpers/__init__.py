...
try:  # FastAPI >=0.99 exposes Request at top-level
    ...
    try:
        ...
        class Request:  # type: ignore
            ...
            def __init__(self, headers: Optional[Dict[str, str]] = None, cookies: Optional[Dict[str, Any]] = None) -> None:
...
def _build_request_context(claims: Optional[Dict[str, Any]], request: Request) -> Dict[str, Any]:
    ...
    request_id = getattr(getattr(request, "state", None), "namel3ss_request_id", None)
...
def _store_request_context(request: Request, context: Dict[str, Any]) -> None:
    ...
    try:
        ...
        if state is not None:
            setattr(state, "namel3ss", merged_context)
            request_id_value = getattr(state, "namel3ss_request_id", None)
    ...
    if isinstance(request_id_value, str) and request_id_value.strip():
        merged_context.setdefault("request_id", request_id_value.strip())
    if _runtime_module is not None:
        ...
        runtime_payload = merged_context
        if callable(getter):
            try:
                existing = getter({})
            except Exception:
            ...
            combined.update(merged_context)
            runtime_payload = combined
        try:
            if callable(setter):
                setter(runtime_payload)
            elif not runtime_payload and callable(clearer):
                clearer()
        except Exception:  # pragma: no cover - defensive
...
async def _enforce_csrf(request: Request) -> None:
    if _runtime_module is None:
        return
    validator = getattr(_runtime_module, "validate_csrf_request", None)
    ...
    try:
        valid = bool(validator(method, headers, cookies))
    except Exception as exc:  # pragma: no cover - defensive
        _LOGGER.exception("CSRF validator failed")
        raise _make_http_exception(403, "CSRF validation failed") from exc
    if not valid:
        raise _make_http_exception(403, "CSRF token missing or invalid")
...
_RATE_LIMIT_DEPENDENCIES: Dict[str, Any] = {}


def rate_limit_dependency(bucket: str) -> Any:
    bucket_key = bucket or "default"
    if bucket_key in _RATE_LIMIT_DEPENDENCIES:
        return _RATE_LIMIT_DEPENDENCIES[bucket_key]

    async def _apply_rate_limit(request: Request) -> None:
        if _runtime_module is None:
            return
        limiter = getattr(_runtime_module, "enforce_rate_limit", None)
        if not callable(limiter):
            return
        identifier = _client_identifier(request)
        try:
            limiter(bucket_key, identifier)
        except Exception as exc:  # pragma: no cover - defensive
            if exc.__class__.__name__ == "RateLimitExceeded":
                detail = f"Rate limit exceeded ({bucket_key})"
                describe = getattr(_runtime_module, "describe_rate_limit", None)
                if callable(describe):
                    config = describe(bucket_key)
                    if isinstance(config, dict) and config.get("limit") and config.get("window"):
                        detail = f"{detail}: {config['limit']} per {config['window']}s"
                raise _make_http_exception(429, detail) from exc
            raise

    dependency = _make_dependency(_apply_rate_limit)
    _RATE_LIMIT_DEPENDENCIES[bucket_key] = dependency
    return dependency
...
def router_dependencies() -> List[Any]:
    ...
    if _runtime_module is not None and hasattr(_runtime_module, "validate_csrf_request"):
        dependencies.append(_make_dependency(_enforce_csrf))
    return dependencies
