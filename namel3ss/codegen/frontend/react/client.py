"""
API client library and data management utilities.

This module generates the n3Client.ts TypeScript library that handles:
- Type definitions for widgets and page data
- React hooks for fetching page data
- Optimistic UI update merging logic
- Widget data resolution from page payloads
"""
...
def write_client_lib(lib_dir: Path) -> None:
    """
    Generate n3Client.ts with comprehensive API client functionality.
    
    This creates a large TypeScript module (~400 lines) containing:
    - Widget configuration type definitions
    - Page definition interfaces
    - usePageData hook for reactive data fetching
    - resolveWidgetData for extracting widget-specific data
    - Optimistic update merging utilities (datasets, rows, summaries)
    """
    ...
    write_file(lib_dir / "n3Client.ts", content)
