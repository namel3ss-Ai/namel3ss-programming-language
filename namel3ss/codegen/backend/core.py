...
# ---------------------------------------------------------------------------
# SQL dataset compilation (shared with generated runtime)
# ---------------------------------------------------------------------------
...
'''
    ...
    "CONNECTOR_DRIVERS: Dict[str, Callable[[Dict[str, Any], Dict[str, Any]], Awaitable[Any]]] = {}",
...
async def _default_grpc_driver(connector: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    The connector ``options`` dictionary may specify:

    - ``host`` (required): target hostname.
    - ``port`` (optional, default ``443``): target port.
    - ``service`` (required): fully qualified gRPC service name.
    - ``method`` (required): RPC method name.
    - ``metadata`` (optional): dict of header metadata.
    - ``payload`` (optional): dict passed to the driver callable.
    - ``deadline_ms`` (optional): request deadline hint.
    - ``driver`` (optional): import path to a callable implementation.
    - ``demo`` (optional): when truthy, enables demo responses.

    Returns a structured dictionary describing the call outcome without raising.
    """
...
async def _default_streaming_driver(connector: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # If we reach here the source type is unsupported.
...
def compile_dataset_to_sql(
    ...
    try:  # Late import keeps runtime lightweight when the AST package is absent.
        ...
        from namel3ss.ast.datasets import GroupByOp  # local import keeps signature clean
...
async def _load_dataset_source(
    ...
    if source_type == "grpc":
        ...
        if driver:
            try:
                response = await driver(connector, context)
                return _extract_rows_from_connector_response(response)
            except Exception:
