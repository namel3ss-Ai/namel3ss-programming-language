...
class DatasetParserMixin(ExpressionParserMixin):
    """Parsing logic for dataset declarations."""
    ...
    def _parse_dataset(self, line: str, line_no: int, base_indent: int) -> Dataset:
        ...
        try:
            ...
            raise self._error(f"Unable to parse dataset declaration: {exc}", line_no, line)

        if len(parts) < 2 or parts[0] != 'dataset':
            raise self._error("Expected: dataset \"Name\" from ...", line_no, line)
        name = parts[1]
        if len(parts) < 4 or parts[2].lower() != 'from':
            raise self._error("Dataset must specify source via 'from'", line_no, line)
        ...
        while self.pos < len(self.lines):
            ...
            if lowered.startswith('filter by:'):
                ...
                if not col_name:
                    raise self._error("Column name cannot be empty", self.pos + 1, nxt)
    ...
    def _parse_dataset_transform_block(self, header_line: str, header_indent: int) -> DatasetTransformStep:
        ...
        try:
            ...
            raise self._error(f"Unable to parse transform declaration: {exc}", self.pos, header_line)
        if len(tokens) < 2 or tokens[0].lower() != 'transform':
            raise self._error("Expected: transform \"Name\"", self.pos, header_line)
    ...
    def _parse_dataset_schema_block(self, parent_indent: int) -> List[DatasetSchemaField]:
        ...
        while self.pos < len(self.lines):
            ...
            if not name_token:
                raise self._error("Column name required inside schema block", self.pos + 1, line)
    ...
    def _parse_dataset_feature(self, header_line: str, header_indent: int) -> DatasetFeature:
        ...
        try:
            ...
            raise self._error(f"Unable to parse feature declaration: {exc}", self.pos, header_line)
    ...
    def _parse_dataset_target(self, header_line: str, header_indent: int) -> DatasetTarget:
        ...
        try:
            ...
            raise self._error(f"Unable to parse target declaration: {exc}", self.pos, header_line)
        if len(tokens) < 2 or tokens[0].lower() != 'target':
            raise self._error("Expected: target \"Name\"", self.pos, header_line)
    ...
    def _parse_dataset_quality_check(self, header_line: str, header_indent: int) -> DatasetQualityCheck:
        ...
        try:
            ...
            raise self._error(f"Unable to parse quality check declaration: {exc}", self.pos, header_line)
        if len(tokens) < 2 or tokens[0].lower() != 'quality':
            raise self._error("Expected: quality \"Name\"", self.pos, header_line)
    ...
    def _parse_connector_option(self, text: str, line_no: int, line: str) -> tuple[str, Any]:
        if not text:
            raise self._error("Connector option requires 'with option key value'", line_no, line)
        key, _, value_text = text.partition(' ')
        key = key.strip()
        if not key:
            raise self._error("Connector option key is missing", line_no, line)
