...
class ComponentParserMixin(ActionParserMixin):
    ...
    def _parse_show_table(self, line: str, base_indent: int) -> ShowTable:
        match = re.match(
            r'show\s+table\s+"([^\"]+)"\s+from\s+(table|dataset)\s+([^\s]+)\s*$',
            line.strip(),
        )
        if not match:
            raise self._error(
                "Expected: show table \"Title\" from table|dataset SOURCE",
                self.pos,
                line,
            )
        title = match.group(1)
    ...
    def _parse_show_chart(self, line: str, base_indent: int) -> ShowChart:
        match = re.match(
            r'show\s+chart\s+"([^\"]+)"\s+from\s+(table|dataset|file)\s+([^\s]+)\s*$',
            line.strip(),
        )
        if not match:
            raise self._error(
                "Expected: show chart \"Title\" from table|dataset|file SOURCE",
                self.pos,
                line,
            )
