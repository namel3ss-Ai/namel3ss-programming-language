...
APP: Dict[str, Any] = {'name': 'Memory Chat Demo', 'database': None, 'theme': {}, 'variables': []}
DATASETS: Dict[str, Dict[str, Any]] = {}
FRAMES: Dict[str, Dict[str, Any]] = {}
CONNECTORS: Dict[str, Dict[str, Any]] = {}
AI_CONNECTORS: Dict[str, Dict[str, Any]] = {}
AI_MODELS: Dict[str, Dict[str, Any]] = {}
LLM_REGISTRY: Dict[str, Dict[str, Any]] = {'gpt4': {'name': 'gpt4',
          'provider': 'openai',
          'model': 'gpt-4o-mini',
          'temperature': 0.7,
          'max_tokens': 800,
...
AI_MEMORIES: Dict[str, Dict[str, Any]] = {'conversation_history': {'name': 'conversation_history',
                          'scope': 'user',
                          'kind': 'list',
                          'config': {'scope': 'user', 'kind': 'list', 'max_items': 100},
                          'metadata': {},
                          'max_items': 100},
 'recent_topics': {'name': 'recent_topics',
                   'scope': 'session',
                   'kind': 'list',
                   'config': {'scope': 'session', 'kind': 'list', 'max_items': 10},
                   'metadata': {},
                   'max_items': 10}}
AI_PROMPTS: Dict[str, Dict[str, Any]] = {'chat_response': {'name': 'chat_response',
                   'model': 'gpt4',
                   'template': '\n'
...
AI_CHAINS: Dict[str, Dict[str, Any]] = {'chat_flow': {'name': 'chat_flow',
               'input_key': 'input',
               'steps': [{'type': 'step',
                          'kind': 'prompt',
                          'target': 'chat_response',
                          'options': {'user_message': {'type': 'member_access',
                                                       'object': 'input',
                                                       'member': 'message'},
                         ...
                         {'type': 'step',
                          ...
                          'options': {'value': {'type': 'member_access',
                                                'object': {'type': 'member_access',
                                                           'object': {'type': 'member_access',
                                                                      'object': 'steps',
                                                                      'member': 'respond'},
                                                           'member': 'output'},
               ...
               'metadata': {'description': 'Loads preferences, generates response, and updates '
                                           'memories'}}}
...
PAGES: List[Dict[str, Any]] = [{'name': 'Chat',
  'route': 'chat',
  'slug': 'chat',
  'index': 0,
  'api_path': '/api/pages/chat',
  'reactive': False,
  'refresh_policy': None,
  'layout': {'direction': 'column', 'spacing': 'medium', 'extras': {}},
  'components': []}]
...
async def page_chat_0(session: Optional[AsyncSession] = None) -> Dict[str, Any]:
    context = build_context('chat')
    scope = ScopeFrame()
    scope.set('context', context)
    instructions = []
    components = await render_statements(instructions, context, scope, session)
    base_api_path = '/api/pages/chat'
    components = await prepare_page_components({'api_path': base_api_path, 'slug': 'chat'}, components, context, session)
    page_errors: List[Dict[str, Any]] = []
    for entry in _collect_runtime_errors(context):
        ...
        scope_value = entry.get('scope')
        normalized_scope = str(scope_value).strip().lower() if scope_value is not None else ''
        if normalized_scope in {'', 'chat', 'page:chat', 'page.chat', 'page'}:
            entry['scope'] = 'page:chat'
        page_errors.append(entry)
    return {
        'name': 'Chat',
        'route': 'chat',
